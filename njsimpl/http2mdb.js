...
function CRUDImpl(tenant) {
  ...
  function MDBRequest(uri) {
    ...
    if (segments.length > 1 && segments[1].length > 0) {
      try {
        ...
        console.log(
          databaseLogPrefix +
            '.MDBRequest(): got exception: ' +
            exception +
            '. This might be due to using an _id that has been assigned remotely: ' +
            segments[1]
        );
  ...
  this.processRequest = function (req, res, apiref) {
    ...
    if (
      req.method == 'POST' &&
      utils.startsWith(req.headers['content-type'], 'multipart/form-data;')
    ) {
      // we first need to create a connection to the db
      console.log(
        databaseLogPrefix +
          '.processRequest(): we have a multipart request. No need to connect to db'
      );
      handleMultipartRequest(uri, req, res);
    } else {
      // we first need to create a connection to the db
      console.log(
        databaseLogPrefix + '.processRequest(): create db connection to: ' + databaseUrl
      );
      mdbclient.connect(
        databaseBaseUrl,
        {
        ...
        function (err, client) {
          if (err) {
            console.error(
              databaseLogPrefix + '.processRequest(): cannot access database: ',
              err
            );
            res.writeHead(500);
            res.end();
          ...
          console.log(
            databaseLogPrefix +
              '.processRequest(): req header user-agent: ' +
              req.headers['user-agent']
          );
          console.log(
            databaseLogPrefix +
              '.processRequest(): req header host: ' +
              req.headers['host']
          );
          ...
          console.log(
            databaseLogPrefix + '.processRequest(): collection from db: ' + collection
          );
          ...
          if (collection) {
            if (req.method == 'GET') {
              ...
              console.error(
                databaseLogPrefix +
                  '.processRequest(): cannot handle request method: ' +
                  req.method
              );
              res.writeHead(405);
              res.end();
            ...
            console.error(
              databaseLogPrefix +
                '.processRequest(): request does not seem to specifiy a collection: ' +
                uri +
                '!'
            );
            res.writeHead(400);
  ...
  function readObject(collection, objectid, req, res, client) {
    ...
    collection.findOne(
      ...
      function (err, element) {
        ...
        if (err) {
          console.log(
            databaseLogPrefix + '.readObject(): Error accessing collection! ' + err
              ? err
              : ''
          );
          respondError(res);
          ...
          console.error(
            databaseLogPrefix +
              '.readObject(): the element with id ' +
              objectid +
              ' could not be found inside of the collection.'
          );
  ...
  function readAllObjects(collection, req, res, client) {
    ...
    collection.find({}).toArray(function (err, elements) {
      ...
      if (err || !elements) {
        console.log(
          databaseLogPrefix +
            '.readAllObjects(): Error accessing collection: ' +
            err +
            '!'
        );
        respondError(res);
        ...
        console.log(
          databaseLogPrefix + '.readAllObjects(): found ' + elements.length + ' elements.'
        );
    ...
    req.on('end', function () {
      ...
      collection.insertOne(parseddata, function (err, result) {
        ...
        if (err || !result) {
          ...
          console.log(
            databaseLogPrefix +
              '.createObject(): saved object id is: ' +
              result.insertedId
          );
      ...
      collection.updateOne(
        ...
        function (err, updated) {
          ...
          if (err || !updated) {
            console.error(
              databaseLogPrefix +
                '.updateObject(): object data could not be updated: ' +
                err
            );
            respondError(res);
          } else {
  ...
  function handleMultipartRequest(uri, req, res) {
    ...
    multipart.handleMultipartRequest(
      req,
      res,
      './www/' + (tenant ? tenant.id + '/' : ''),
      'content/',
      function ondone(formdata) {
        console.log(
          databaseLogPrefix +
            '.handleMultipartRequest(): got formdata: ' +
  ...
  function respondMultipart(req, res, uri, content) {
    ...
    if (content.createIframeCallback) {
      ...
      var script =
        '<script language="javascript" type="text/javascript">var content = ' +
        JSON.stringify(content) +
        "; window.top.window.vc.onMultipartResponse('" +
        uri +
        "', content);</script>";
      res.writeHead(200, {
