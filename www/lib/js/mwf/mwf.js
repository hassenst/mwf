...
function ApplicationState() {
  ...
  this.popViewController = function (returnData, returnStatus) {
    ...
    console.log(
      'REFACVIEWS: popViewController(): ' + vc.root.id + '. Will call onpause+onstop',
      vc
    );
        ...
        vc.onstop().then(
          function () {
            ...
            if (countdown == 0) {
              ...
              console.log(
                'clearControllers(): countdown is greaterorequal than one: ' + countdown
              );
            }
          }.bind(this)
        );
...
function ApplicationResources() {
  ...
  this.getTemplateInstance = function (templatename) {
    ...
    if (!template) {
      console.error(
        'getTemplateInstance(): the template ' + templatename + ' does not exist!'
      );
    } else {
...
class ViewController {
  ...
  setArgs(args) {
    ...
    console.log(
      'ViewController.setArgs(): args: ' + (args ? mwfUtils.stringify(args) : ' no args')
    );
  ...
  onListItemElementSelected(listitemEl, listview) {
    console.log(
      'ViewController.onListItemElementSelected(): ' + listitemEl + ' from ' + listview
    );
  ...
  updateActionmenuItems(menu) {
    ...
    console.log(
      'updateActionmenuItems(): items are: ' + mwfUtils.stringify(this.actionmenuItems)
    );
  }
...
class SidemenuViewController extends EmbeddedViewController {
  ...
  attachToView(view) {
    ...
    console.log(
      'SideMenuViewController: found mainmenu elements: ' + mainmenuActionElements.length
    );
...
class Application {
  ...
  registerCRUD(entitytype, scope, impl) {
    console.log(
      'registerCRUD(): crudops declaration for ' + entitytype + ' in scope ' + scope
    );
  ...
  initialiseCRUD(scope, em) {
    ...
    console.log(
      'initialiseCRUD(): crudops declaration is: ' + mwfUtils.stringify(this.crudops)
    );
...
function onloadApplication() {
  ...
  if (stylingElements.length > 0) {
    ...
    for (m = 0; m < embeddedComponents.length; m++) {
      currentComponentViewControllerClassname = embeddedComponents[m].getAttribute(
        'data-mwf-viewcontroller'
      );
      console.log(
